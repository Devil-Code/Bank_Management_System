...
public class BankDAO
{
	...
	public int updateBank(BankBean bb)
	{
		...
		{
			con=BankDB.getConnection();
			String ins_str = "update bankEmployee set Employee_Name=?,Father_Name=?,Gender=?,Dob=?,Address=?,Phone_Number=?,Email_ID=?,Marital_Status=? where ID=?;";
			PreparedStatement p = con.prepareStatement(ins_str);
			p.setString(1,bb.getEname());
			p.setString(2,bb.getFname());
			p.setString(3,bb.getGender());
			p.setDate(4,bb.getDob());
			p.setString(5,bb.getAdd());
			p.setString(6,bb.getPhone());
			p.setString(7,bb.getEmail());
			p.setString(8,bb.getMarital());
			p.setInt(9,bb.getId());
			int updateCount = p.executeUpdate(); 
			return updateCount; 
		} catch(Exception ex){
	...
	public ResultSet loadTable()
	{
		...
		try {
			con=BankDB.getConnection();
			String sql="Select ID,Employee_Name,Gender,Phone_Number,Blood_Group from bankEmployee;";
			PreparedStatement p=con.prepareStatement(sql);
			ResultSet rs = p.executeQuery();
			if(rs.next()==false)
				return null;
	...
	public ResultSet ShowSp(int id)
	{
		try {
			...
			String sql="Select Employee_Name,Father_Name,Gender,Dob,Blood_Group,Pan_no,Addhar_no,Address,Phone_Number,Email_ID,Marital_Status from bankEmployee where ID=?;";
			PreparedStatement p=con.prepareStatement(sql);
			p.setInt(1,id);
			ResultSet rs = p.executeQuery();
			return rs;
